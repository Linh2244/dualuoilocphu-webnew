...
## Deploy / verify
- The site runs on a **PHP host** (cPanel/shared, InfinityFree, XAMPP locally). The homepage is now a PHP page (`index.php`) and **cannot run on GitHub Pages** — a `CNAME` → `web.linh.qzz.io` may still exist in the repo, but GH Pages would serve `index.php` as raw source, so don't rely on it. No build step.
...
- Verify static layout by opening `.html` files directly; verify the PHP pages (`index.php`, `admin/*`) by serving the folder from a PHP-capable server (e.g. XAMPP `htdocs`) and visiting `/admin/setup.php`, then the site pages.
